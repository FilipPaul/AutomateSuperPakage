...
from AutomateSuperPackage.HardwarePackage.FlashRunnerPackage.YMTask import SendTask, ReceiveTask
...
class YModem(object):
    ...
    def send(self, data_stream, data_name, data_size, retry=20, callback=None):
        ...
        self.putc(EOT)
        self.log.debug(">>> EOT")
        self.wait_for_next(ACK)#changed from nack
        self.putc(EOT)
        self.log.debug(">>> EOT")
        self.wait_for_next(ACK)
    ...
    def recv_file(self, outputfile, callback=None):
        while True:
            self.putc(CRC)
            self.log.debug("<<< CRC")
            c = self.getc(1)
                ...
                if not (seq == seq_oc == sequence):
                    ...
                    if valid:
                        ...
                        if seq == 0 and not FIRST_PACKET_RECEIVED and not WAIT_FOR_END_PACKET:
                            ...
                            pos = data.find(b'\x00')
                            pos = pos + 1
                            pos2 = data[pos :].find(b'\x00')
                            file_name_bytes =  data[: pos]
                            data_size_bytes = data[pos :pos + pos2]
                            ...
                            file_stream = open(outputfile, 'wb+')
                            #file_stream = open("outputLog.txt", 'wb+')
                            FIRST_PACKET_RECEIVED = True
                            sequence = (sequence + 1) % 0x100
                            #file_stream = open("output.txt", 'wb+')
